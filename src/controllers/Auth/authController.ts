...
export const register = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    if (existingUser) {
      logger.warn('Failed registration attempt - Email already exists', {
        email,
    ...
    logger.error('System error during user registration', {
      error: error.message,
      stack: error.stack,
...
export const verifyEmail = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    if (!user) {
      logger.warn('Failed email verification attempt - Invalid or expired token', {
        token,
      ...
      <html>
        <body style="font-family: sans-serif; display: flex; align-items: center; justify-content: center; height: 100vh; background-color: #f9fafb;">
          <div style="text-align: center; background: white; padding: 40px; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
            ...
            }/login" style="display: inline-block; margin-top: 20px; padding: 10px 20px; background-color: #10b981; color: white; text-decoration: none; border-radius: 8px; font-weight: bold;">Go to Login</a>
          </div>
        </body>
      </html>
    `);
...
export const login = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    logger.info('Successful standard login', {
      email,
      userId: user._id,
...
export const googleLogin = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    try {
      ...
      logger.warn('Failed Google Login verification', {
        error: error.message,
    ...
    const { email, name, sub } = payload as any;
...
export const refreshToken = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    if (!storedUserId || storedUserId !== userId.toString()) {
      logger.warn('Failed Refresh Token rotation - Token inactive or stolen', {
        userId,
        tokenId,
