...
export const saveCallOutcome = async (
  ...
  if (input.substageId) {
    selectedSubstage = await (prisma as any).leadSubstage.findFirst({
    ...
    if (targetStage && targetStage.id !== lead.stageId) {
      ...
      if (targetStage.isApprovalRequired) {
        ...
        const approval = await (prisma as any).leadStageApproval.create({
          data: {
            workspaceId,
            leadId,
        ...
        if (input.stageRuleValues && input.stageRuleValues.length > 0) {
          ...
          await (prisma as any).leadStageInput.createMany({ data: inputsToSave });
  ...
  const outcome = await (prisma as any).leadCallOutcome.create({
    data: {
      callSessionId: callSession.id,
      workspaceId,
      leadId,
