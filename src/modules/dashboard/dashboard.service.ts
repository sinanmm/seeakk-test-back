...
import { closedModuleLeadWhere } from '../leads/leadVisibility.util';
import { mergeWorkspaceLeadFilters } from '../leads/leadQueryScope';
import { getDayRangeForWorkspace } from '../../services/User/followupService';
import type { DashboardSummaryQueryInput, RevenueAnalyticsQueryInput, ProductAnalyticsQueryInput } from './dashboard.validation';
...
const calculateDashboardRevenue = async (
  ...
  const closedWhere = closedModuleLeadWhere();
  const andConditions: Prisma.LeadWhereInput[] = [dashboardFilters, closedWhere];

  if (dateWindow) {
    andConditions.push({
      OR: [
        { closedAt: { gte: dateWindow.gte, lte: dateWindow.lte } },
        { closedAt: null, updatedAt: { gte: dateWindow.gte, lte: dateWindow.lte } },
      ],
    });
  }
  ...
  const where = mergeWorkspaceLeadFilters(workspaceId, leadAccess, extraWhere);

  const [aggregateResult, count] = await Promise.all([
  ...
  const sum = Number(aggregateResult._sum?.totalAmount || 0);
  return { sum, count };
};
...
export const getDashboardSummary = async (
  ...
  const { start: scheduleTodayStart, end: scheduleTodayEnd } = await getDayRangeForWorkspace(workspaceId);
  const scheduleUserId = query.userId || actor.id;
  ...
  const results = await Promise.allSettled([
    ...
    dashboardRepository.findTodayFollowUps(workspaceId, scheduleTodayStart, scheduleTodayEnd, 10, scopedLeadAccess, scheduleUserId),
  ...
  return {
    ...
    meetings: followUps.map((item) => ({
      ...
      scheduledAt: item.scheduledAt ? item.scheduledAt.toISOString() : undefined,
    })),
    scheduleDateLabel: formatScheduleLabel(scheduleTodayStart),
    range: query.range,
    pendingApprovals,
    expectedRevenue: expectedRevenue.amount,
    formatted: {
...
export const getProductAnalytics = async (
  workspaceId: string,
  actor: Actor,
  query: ProductAnalyticsQueryInput,
) => {
  await ensureModuleReady();

  // 1. Check if configured products exist in workspace
  const totalProductsConfigured = await prisma.product.count({
    where: {
      workspaceId,
      deletedAt: null,
    },
  ...
  // 2. Fetch all configured products in workspace
  ...
  // 3. Build lead access and filter criteria
  const leadAccessWhere = await buildAccessWhere(workspaceId, actor);
  const dashboardFilters = buildDashboardLeadFilters(query);
  const leadWhere = mergeWorkspaceLeadFilters(workspaceId, leadAccessWhere, dashboardFilters);

  // 4. Fetch LeadProducts attached to matching leads
  ...
  // Initialize with all configured products
  products.forEach((p) => {
  ...
  // Aggregate lead product items
  leadProducts.forEach((lp) => {
    const key = lp.productId || lp.productName;
    if (!productStatsMap.has(key)) {
      productStatsMap.set(key, {
        id: lp.productId || lp.productName,
        name: lp.productName,
        code: null,
        quantitySold: 0,
        revenue: 0,
    ...
    const itemRevenue = lp.lineTotal ?? qty * (lp.unitPrice || 0);

    stat.quantitySold += qty;
    stat.revenue += itemRevenue;

    if (lp.lead) {
      stat.leadIds.add(lp.lead.id);
      if (lp.lead.isClosed) {
        stat.closedLeadIds.add(lp.lead.id);
      } else if (!lp.lead.isLOB) {
        stat.openLeadIds.add(lp.lead.id);
      }
    }
  });
  ...
  const performanceTable = Array.from(productStatsMap.values()).map((stat) => {
    productsSold += stat.quantitySold;
    totalRevenue += stat.revenue;

    const totalLeadsCount = stat.leadIds.size;
    const closedLeadsCount = stat.closedLeadIds.size;
    const openLeadsCount = stat.openLeadIds.size;
    const averageSale = stat.quantitySold > 0 ? stat.revenue / stat.quantitySold : 0;
    const conversionRate = totalLeadsCount > 0 ? (closedLeadsCount / totalLeadsCount) * 100 : 0;
  ...
  const ranking = [...performanceTable].sort((a, b) => b.revenue - a.revenue);

  // Best & Lowest Selling Products
  const productsWithSales = ranking.filter((p) => p.quantitySold > 0);
  const bestSellingProduct = productsWithSales.length > 0 ? productsWithSales[0] : null;
  const lowestSellingProduct =
    productsWithSales.length > 0 ? productsWithSales[productsWithSales.length - 1] : null;

  const averageProductValue = productsSold > 0 ? Math.round((totalRevenue / productsSold) * 100) / 100 : 0;

  // Donut Chart: Revenue Contribution
  const revenueContribution = ranking
  ...
  const totalDays = range === '30d' ? 30 : range === '12m' ? 365 : 7;
  const now = new Date();

  // Create trend buckets
  const trendPeriods = Array.from({ length: range === '12m' ? 12 : totalDays }, (_, index) => {
    if (range === '12m') {
      const monthDate = startOfMonth(addMonths(now, index - 11));
      const key = `${monthDate.getFullYear()}-${String(monthDate.getMonth() + 1).padStart(2, '0')}`;
      const label = new Intl.DateTimeFormat('en-US', { month: 'short', year: '2-digit' }).format(monthDate);
      return { key, label, revenue: 0, units: 0 };
    }

    const dayDate = startOfDay(addDays(now, index - (totalDays - 1)));
    const key = dayDate.toISOString().slice(0, 10);
    const label =
      range === '30d'
        ? new Intl.DateTimeFormat('en-US', { month: 'short', day: 'numeric' }).format(dayDate)
  ...
  const trendIndexMap = new Map(trendPeriods.map((item, index) => [item.key, index]));

  leadProducts.forEach((lp) => {
    const itemDate = lp.createdAt || new Date();
    const key =
      range === '12m'
        ? `${itemDate.getFullYear()}-${String(itemDate.getMonth() + 1).padStart(2, '0')}`
        : itemDate.toISOString().slice(0, 10);

    const idx = trendIndexMap.get(key);
    if (idx !== undefined) {
      const qty = lp.quantity || 1;
      const itemRevenue = lp.lineTotal ?? qty * (lp.unitPrice || 0);
      trendPeriods[idx].revenue += itemRevenue;
      trendPeriods[idx].units += qty;
    }
  });

  const revenueTrend = trendPeriods.map((item) => ({
    name: item.label,
  ...
  return {
    hasProducts: true,
    totalProducts: totalProductsConfigured,
    productsSold,
    ...
    lowestSellingProduct: lowestSellingProduct
      ? {
          id: lowestSellingProduct.id,
          name: lowestSellingProduct.name,
          quantitySold: lowestSellingProduct.quantitySold,
          revenue: lowestSellingProduct.revenue,
        }
      : null,
    averageProductValue,
    ranking,
