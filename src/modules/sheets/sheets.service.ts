...
export const syncLeadChanges = async (workspaceId: string, actor: Actor, input: SyncSheetInput) => {
  ...
  activeDynamicFields.forEach((f: any) => {
    if (f.id) dynamicFieldKeys.add(normalizeKey(f.id));
    if (f.name) dynamicFieldKeys.add(normalizeKey(f.name));
  });

  const SUPPORTED_KEYS = new Set<string>([
    'stage', 'stageid', 'current lead stage',
    'name', 'lead name',
    'email', 'phone', 'mobile', 'mobile number',
    'companyname', 'company name', 'address',
    'expectedrevenue', 'expected revenue contribution', 'expected revenue',
    'totalamount', 'total amount',
    'advanceamount', 'advance amount', 'approved advance amount',
    'balanceamount', 'balance amount',
    'remarks',
    'assigneduser', 'assignedtoid', 'assigned user', 'assigned to',
    'source', 'sourceid', 'lead source',
    'lifecycle', 'lifecycleid', 'lead lifecycle',
    'products', 'product',
    'nextfollowupat', 'nextfollowupdate', 'followup', 'next follow up', 'next follow up at', 'followup date', 'follow-up date'
  ]);
  ...
  if (changesList.length === 0) {
    ...
    return {
      ...
      pending,
      blocked,
      summary: { totalLeads: 0, appliedCount: 0, pendingCount: 0, blockedCount: 0 },
      message: 'No changes detected to sync.',
    };
  }
  ...
  return {
    ...
    message: `Sync completed. ${applied.length} change(s) applied, ${pending.length} pending approval, ${blocked.length} failed.`,
  };
};
